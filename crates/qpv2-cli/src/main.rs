...
fn resolve_auth_key(wallet_id: u32, purpose: &str) -> Result<AuthKey, String> {
    ...
    match wallet_info.auth_method {
        AuthMethod::Password => {
            let password =
                prompt_for_input(&format!("Enter your wallet password to {}: ", purpose))?;
            Ok(AuthKey::Password(password))
        }
        AuthMethod::Keychain => {
            println!("Authenticate with {}...", keychain::short_name());
            let key = keychain::retrieve_key(wallet_id, purpose)?;
        ...
        AuthMethod::Fido2 { ref credential_id } => {
            ...
            let pin =
                prompt_for_input(&format!("Enter your security key PIN to {}: ", purpose))?;
